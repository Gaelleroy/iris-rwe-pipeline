...
--
-- ROW_NUMBER ordered only by distance-to-target is ambiguous when two
-- measurements are equidistant: the SQL standard leaves the winner arbitrary,
-- and DuckDB and Trino resolve it differently. In this cohort 10 patients have
-- two follow-up measurements the same number of days either side of the
-- 365-day target with different acuity values, which was enough to shift the
-- odds ratios in the third decimal between the local and Athena runs while the
-- cohort count stayed identical. Every ORDER BY that picks one row out of many
-- therefore carries explicit secondary keys.
--
