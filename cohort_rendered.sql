...
--
-- Placeholders: 'E11.311', 'E11.321', 'E11.331', 'E11.341', 'E11.351', 'H35.81' 'Drug A', 'Drug B' 18 95 365
--               365 90 30 0.1
--
...
dme_dx AS (
    ...
    FROM diagnoses
    WHERE diagnosis_code IN ('E11.311', 'E11.321', 'E11.331', 'E11.341', 'E11.351', 'H35.81')
    GROUP BY patient_id
),
...
study_inj AS (
    ...
    FROM injections
    WHERE medication IN ('Drug A', 'Drug B')
),
...
prior_use AS (
    ...
    WHERE i.injection_date < e.index_date
      AND i.injection_date >= e.index_date - INTERVAL '365' DAY
),
...
baseline_va AS (
    ...
    FROM (
        ...
        WHERE v.logmar IS NOT NULL
          ...
          AND v.measurement_date >= b.index_date - INTERVAL '30' DAY
    ) t
    WHERE rn = 1
),
        ...
        SELECT v.patient_id,
               ...
               ROW_NUMBER() OVER (
                   ...
                   ORDER BY ABS(DATE_DIFF('day', v.measurement_date,
                                          b.index_date + INTERVAL '365' DAY))
               ) AS rn
        FROM visual_acuity v
        JOIN base b ON v.patient_id = b.patient_id
        WHERE v.logmar IS NOT NULL
          AND v.measurement_date >= b.index_date + INTERVAL '365' DAY
                                   - INTERVAL '90' DAY
          AND v.measurement_date <= b.index_date + INTERVAL '365' DAY
                                   + INTERVAL '90' DAY
    ) t
...
improvement AS (
    SELECT b.patient_id,
           MIN(CASE WHEN bv.baseline_logmar - v.logmar >= 0.1
...
inj_counts AS (
    ...
    JOIN study_inj i
     ...
     AND i.injection_date <= b.index_date + INTERVAL '365' DAY
    GROUP BY b.patient_id
)
...
SELECT
    ...
    CASE WHEN bv.baseline_logmar - fv.followup_logmar >= 0.1
...
WHERE b.age_at_index >= 18
  AND b.age_at_index <  95
